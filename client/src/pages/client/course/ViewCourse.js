...
const ViewCourse = () => {
  ...
  const itemsPerPage = 3; // Adjust the number of reviews per page
  ...
  const getLessonData = async (id) => {
    ...
    try {
      const res = await axiosInstance.get(
        `${port}/gettingCourseLessonDataWithSectionId/${id}`
      );
  ...
  const getAllLessonData = async () => {
    try {
      const res = await axiosInstance.get(
        `${port}/gettingCourseLessonDataWithCourseId/${id}`
      );
      const lessonquizdata = res.data;
      // const filterOrderData = lessonquizdata.filter((item) => item.order !== 0);
      const sortedData = lessonquizdata.sort((a, b) => a.order - b.order);
  ...
  const getTotalEnroll = async () => {
    try {
      const response = await axiosInstance.get(
        `${port}/gettingEnrollWithCourseId/${id}`
      );
  ...
  const getCourseCategory = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${port}/gettingCoureseCategoryWithId/${courseData.course_cate}`
      );
      const data = await response.data;
  ...
  const getViewCourseData = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${port}/gettingCourseMasterDataWithId/${id}`
      );
      const data = await response.data;
      setCourseData(data);
      //long desc
      let long_desc = data.long_desc;
      if (typeof long_desc === "string") {
        long_desc = long_desc.replace(/^"|"$/g, ""); // Removes only the first and last quotes
      }
      ...
      response.data.course_topics = Array.isArray(what_you_will_learn)
        ? what_you_will_learn
        : [];
      //course requirements
      ...
      response.data.course_requirenment = Array.isArray(course_requirements)
        ? course_requirements
        : [];
      //course faqs
  ...
  const isInCart = cart.some((item) =>
    savedToken ? item.course_id == courseData.id : item.id == courseData.id
  );
  //get module data
  ...
  const getModuleData = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${port}/gettingCourseSectionData/${id}`
      );
  ...
  // Get the current reviews slice based on pagination
        ...
        <div className="client_section">
          <div className="main-section p-4 sm:p-8 md:p-12 lg:p-16 lg:gap-16 2xl:flex xl:flex lg:flex block">
            <div className="header-content 2xl:w-8/12 xl:w-8/12 lg:w-8/12 w-full">
              ...
              <h2 className="font-bold mb-2">{courseData?.course_title}</h2>
              <p className="course-description">{courseData?.short_desc}</p>
              ...
              <div className="course-icon-section">
                <span>
                  <i className="fa-solid fa-copy"></i> {allLessonData?.length}{" "}
                  Lessons
                </span>
                <span>
                  <i className="fa-solid fa-clock"></i>{" "}
                  {totalTime ? totalTime : 0} Hours
                </span>
              ...
              {activeTab === "overview" && (
                <>
                  <div className="long-desc">
                    {courseData?.long_desc == "undefined" ? (
                      ""
                    ) : (
                      <p
                  ...
                  <div className="learning-list">
                    ...
                    <ul>
                      {Array.isArray(courseData.course_topics) &&
                        courseData.course_topics.map((keyword, index) => (
                  ...
                  <div className="prerequisites">
                    ...
                    <ul className="list-disc pl-4 md:pl-6 space-y-2">
                      {Array.isArray(courseData.course_requirenment) &&
                        courseData.course_requirenment.map((keyword, index) => (
                ...
                <>
                  {courseData.course_faqs.length > 0 ? (
                    <div className="faqs-header">
                      {Array.isArray(courseData.course_faqs) &&
                        courseData.course_faqs.map((keyword, index) => {
                          if (
            ...
            <div className="course-image 2xl:w-4/12 xl:w-4/12 lg:w-4/12 w-full">
              ...
              <div className="price mb-8">
                ...
                <div className="cart-buttons">
                  <button
                    className={`btn-add ${isInCart ? "disabled" : ""}`}
